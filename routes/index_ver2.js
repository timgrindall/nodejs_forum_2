...
router.get('/', function(req, res, next) {
	...
	models.forums.find({}, function (err, doc) {
		...
		models.subforums.find({parentID: new ObjectId('5bf9e49c132cfe4e1c3edddb')}, function(err, firstDoc) {

			return new Promise(function(resolve, reject) {
				resolve(firstDoc);
				if (err) reject(err);
			});
		}).find({parentID: new ObjectId('5bf9e49c132cfe4e1c3edddc')}, function(err, secondDoc) {
			...
			return new Promise(function(resolve, reject) {
				...
				resolve([firstDoc, secondDoc]);

				if (err) {
					reject(err);
				}
			});
		}).find({parentID: id_3}, function(err, thirdDoc) {
